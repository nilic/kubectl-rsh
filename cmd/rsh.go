...
var (
	rshUsageStr    = "kubectl rsh [-c CONTAINER] [flags] (POD | TYPE/NAME) COMMAND [args...]"
	...
	rshLong = templates.LongDesc(`
		...
		Note, some containers may not include a shell - use 'kubectl exec' if you need to run commands
		directly.`)
...
func NewCmdRsh(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().BoolVarP(&printVersion, "version", "v", false, "Print version information")
	...
	//    kubectl rsh must-gather-rddfk rsync --server --sender -vlDtpre.iLsfxC --numeric-ids ...
...
func (o *RshOptions) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	// 'kubectl rsh pod -- date' nor 'kubectl rsh -f pod.yaml -- date'
	...
	// kubectl rsh always provides the command as the second item of args.
	if len(args) > 1 {
		argsLenAtDash = 1
	}
